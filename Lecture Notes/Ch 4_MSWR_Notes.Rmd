...
Let's revisit the coin flip example: What we did is a simulation. Now we can check how close our simulation results are to the theoritical results.
...
*Question*:
  ...
  Now that we know the theoritical sampling distribution of $\bar{X}$, 
    
  1. Find the theoritical sampling distribution of $\bar{X}$ for question 1 above and compare those with the simulated results.
  
  2. Find the theoritical sampling distribution of $\bar{X}$ for question 2 above and compare those with the simulated results.
...
A potential buyer intends to take a sample of four engines and will not place an order if the sample mean is less than 215 HP. What is the probability that the buyer will not place an order?  
  ...
  1. Answer the question using a theoritical distribution:
...
for (i in 1:10000)
{
  ...
  samp <- sample(population, size = 4, replace = TRUE)
  Xbars[i] <- mean(samp)
}
...
Suppose the heights of boys in a certain city follow a distribution with mean 48 in and variance $9^2$. 

  1. Use the CLT to estimate the probability that in a random sample of 30 boys, the mean height is morethan 51 in.
...
4. Let $X_1, X_2, \ldots , X_{10} \overset{i.i.d}\sim N(20, 8)$ and $Y_1, Y_2, \ldots, Y_{15} \overset{i.i.d}\sim N(16, 7)$.  Let $W = \bar{X} + \bar{Y}.$
  ...
  a. Simulate the sampling distribution in `R` and plot your results.  Check that the simulated mean and standard error are close to the theoretical mean and standard error.
      
  b. Use your simulation to find $P(W < 40).$  Calculate an exact answer and compare.
      
  c. Give the exact sampling distribution of $W$
...
The exact sampling distribution of $W$ is $N\left(20 + 16, \sqrt{8^2/10 + 7^2/15}\right) = N(`r 20+16`, `r round(sqrt(8^2/10 + 7^2/15),2)`)$. 
