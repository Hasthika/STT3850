...
mean(W < 40) # from the simulation

pnorm(40, mean = 36, sd = 3.11) # from the exact distribution
...
Consider the population {3, 5, 6, 6, 8, 11, 13, 15, 19, 20}.
  ...
  a) Compute the median of this population.
...
```
  ...
  b) Simulate the sampling distribution of $median$ by taking random samples of size 4 and plot your results. 
  
  d) Does the sampling distribution of $median$ look Normal?
  
  c) Use the simulation to find $P(Median < 9.5)$
...
ggplot(data = data.frame(x = Medians), aes(x = x, fill = x <= 9.5)) + 
...
Consider the population {3, 5, 6, 6, 8, 11, 13, 15, 19, 20}.
  ...
  a) Compute the maximum of this population.
...
```
  ...
  b) Simulate the sampling distribution of $maximum$ by taking random samples of size 5 and plot your results. 
  
  d) Does the sampling distribution of $maximum$ look Normal?
  
  c) Use the simulation to find $P(maximum < 15)$
...
ggplot(data = data.frame(x = Maxes), aes(x = x, fill = x <= 15)) + 
