...
```{r , label = "dist2", fig.cap = "Permutation Distribution of $\\bar{X}_m - \\bar{X}_f$"}
library(ggplot2)
ggplot(data = data.frame(md = ans), aes(x = md)) + 
...
Time_group <- Verizon %>%
  group_by(Group) %>%
  summarize(Average = mean(Time), NumCus = n())
Time_group

MD <- Time_group[2,2] - Time_group[1,2]
MD

observed <- MD$Average   # We had to create a new variable called observed, because MD was a matrix in this case.
observed
